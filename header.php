...
<?php  $ip_short = explode(".",$ip);
			  $ip_short[3];
			 ?>
<?php $arr_ip_it = array( '99', '118', '21', '44', '222', '225', '9', '113' ); ?>



<?php if(is_single()) { ?>

     <?php if(get_field('kythuat')!=1) { ?>

          <?php  if (!in_array($ip_short[3], $arr_ip_it)) { ?>
                <script>
                  if(confirm('Bạn không có quyền truy cập vào hệ thống dữ liệu của bộ phận IT.')) {
                    location.href = 'http://rakuji.vn/';
                  } else {
                    location.href = 'http://rakuji.vn/';
                  }
                </script>
...
<?php } else { 
  if (!in_array($ip_short[3], $arr_ip_it)) { ?>
      <script>
          if(confirm('Bạn không có quyền truy cập vào hệ thống dữ liệu của bộ phận IT.')) {
            location.href = 'http://rakuji.vn/';
          } else {
